<template>
  <div class="teacher-progress-units">
      ...
      <el-col v-for="unit in units" :key="unit.id" :span="8">
        <el-card class="unit-card" shadow="hover" @click="goToUnitDetail(unit.id)">
          <div class="unit-header">
            <h3>{{ unit.title }}</h3>
            <el-tag type="primary" size="small">单元 {{ unit.order }}</el-tag>
          </div>
          ...
          <div class="unit-footer">
            <el-button type="primary" size="small" @click.stop="goToUnitDetail(unit.id)">
              查看进度
...
const goToUnitDetail = (unitId) => {
  router.push(`/teacher/courses/${route.params.uuid}/progress/units/${unitId}`)
}
